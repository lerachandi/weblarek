...
- `protected handleCartButtonClick(): void` — обработчик клика по кнопке корзины, генерирует событие `basket:open`
...
- `set items(elements: HTMLElement[]): void` — ендерит список карточек товаров. Отображение надписи «Корзина пуста» реализовано стилями и не управляется кодом компонента
...
- `set payment(value: string)` — активирует выбранный способ оплаты
- `set address(value: string)` — устанавливает текст в поле адреса доставки
- `private updateValidity(): void` — проверяет корректность заполнения полей; включает/выключает кнопку «Далее» и выводит сообщение об ошибке (ошибка по оплате показывается только после первого выбора/смены оплаты)

**Обработчики событий:**

- клик по кнопке оплаты — генерирует `order:payment-change` и вызывает `updateValidity()`
- ввод адреса — генерирует `order:address-change `и вызывает `updateValidity()`
- отправка формы — генерирует `order:submit`

#### `ContactsFormData`

Форма оформления заказа: e-mail и телефон.
...
- `private updateValidity(): void` — проверяет заполненность полей; включает/выключает кнопку отправки и выводит сообщение об ошибке, но только после первого изменения соответствующего поля (логика touched-полей)

**Обработчики**

- ввод e-mail — генерирует contacts:email-change и вызывает updateValidity()
- ввод телефона — генерирует contacts:phone-change и вызывает updateValidity()
...
- подписывается на события, приходящие от View
- вызывает методы моделей при изменении данных
- обновляет интерфейс при изменениях в моделях
- управляет модальными окнами и переходами между шагами оформления заказа
...
- генерирует `catalog:changed`
- по событию выполняется рендер каталога в `GalleryData`

**Просмотр товара**

- по событию `product:select` создаётся компонент `CardPreviewData`
- карточка открывается в модальном окне
- кнопка внутри карточки генерирует: `product:add`, `product:remove`

**Добавление и удаление товара из корзины**

- по `product:add` товар добавляется в `Basket`
- по `product:remove` товар удаляется
- презентер генерирует `basket:changed`
- UI обновляется: счётчик в HeaderData; корзина, если она открыта; карточка товара в модалке (кнопка «Купить» / «Удалить из корзины»)

**Корзина**

- `basket:open` — презентер рендерит компонент `BasketData`, помещает его в модальное окно и открывает модалку.
- `product:remove` — товар удаляется из модели `Basket`, после чего генерируется `basket:changed`.
- `basket:changed` — обновляет счётчик в шапке и, если корзина открыта, перерисовывает её содержимое.
- `basket:checkout` — модалка остаётся открытой, но её содержимое заменяется на форму `OrderFormData` (первый шаг оформления заказа).
