...
/**
 * Клонирует <form> внутри template
 */
export function cloneFormTemplate<T extends HTMLFormElement>(id: string): T {
  const raw = cloneTemplate<HTMLElement>(id);

  if (raw instanceof HTMLFormElement) {
    return raw as T;
  }

  const form = raw.querySelector("form");
  if (!form) throw new Error(`Template ${id} contains no form`);
  return form as T;
}
